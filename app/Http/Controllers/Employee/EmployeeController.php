...
class EmployeeController extends Controller
{
    public function index() {
        $employee = Auth::user()->employee()->with(['attendance', 'leave', 'expense', 'department'])->first();

        $stats = [
            'today_attendance' => $employee->attendance->filter(function ($item) {
                return optional($item->created_at)->isToday();
            })->count(),
            'pending_leaves' => $employee->leave->where('status', 'pending')->count(),
            'pending_expenses' => $employee->expense->where('status', 'pending')->count(),
        ];
